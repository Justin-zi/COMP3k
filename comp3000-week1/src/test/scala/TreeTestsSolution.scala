...
class TreeTests extends FunSuite {
     ...
     * description and a body.  The body uses the expect function to
    ...
    test ("the sum of a a one-level tree is the sum of the two leaves") {
        assertResult (130) (sumleaves (Fork (Leaf (42), Leaf (88))))
    }
    ...
    test ("the incandswap of a a one-level tree is inc and swap of those leaves") {
        assertResult (Fork (Leaf (89), Leaf (43))) (
            incandswap (Fork (Leaf (42), Leaf (88)))
        )
    }

    test ("the incandswap of a mutli-level tree is as expected") {
        val t = Fork (Leaf (1), Fork (Fork (Leaf (2), Leaf (3)), Leaf (4)))
